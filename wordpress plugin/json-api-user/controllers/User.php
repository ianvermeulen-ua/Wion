...
$allowed_params = array('user_login', 'user_email', 'user_pass', 'display_name', 'user_nicename', 'user_url', 'nickname', 'first_name',
						 ...
						 'comment_shortcuts', 'admin_color', 'use_ssl', 'show_admin_bar_front', 'study', 'year'
                   );
...
//d($data);
	  ...
	  public function request_user_membership() {
		global $json_api;
		...
		if (!$user_id) 	
			$json_api->error("Invalid cookie. Use the `generate_auth_cookie` method. Already done tho xddddd");

		$data['updated'] = wp_update_user( array( 'ID' => $user_id, 'role' => 'nietbetaald' ) );

		if ( is_wp_error( $user_id ) ) {
			$json_api->error("There was an error, probably that user doesn't exist.");
		}
